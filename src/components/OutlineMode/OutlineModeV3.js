...
const OutlineModeV3 = ({
  ...
  // Quickfill props removed for now
}) => {
  ...
  // Note: Quickfill functionality has been removed for now and will be reintroduced later
  ...
  // Check if a column can be deleted (all columns except row title can be deleted)
  const canDeleteColumn = (colIdx) => {
    // Can't delete row title column
    if (colIdx === 0) return false;
    
    // Any other column can be deleted
    return true;
  };

  // Check if a column can be instantly deleted (all values are empty)
  const canDeleteColumnInstant = (colIdx) => {
    // Can't delete row title column
    if (colIdx === 0) return false;
    
    // Check if all values in the column are empty
    const cellIdx = colIdx - 1; // Adjust for row title column
    
    // Check all rows in all sections for empty values at this column index
    for (const section of sectionData.rowSections) {
      for (const row of section.rows) {
        if (row.cells && row.cells[cellIdx]) {
          const cellValue = row.cells[cellIdx].value;
          if (cellValue && cellValue.trim() !== '') {
            return false; // Found a non-empty value, can't delete instantly
          }
        }
      }
    }
    
    // All values in this column are empty, can delete instantly
    return true;
  };

  // Check if a row can be instantly deleted (all values are empty)
  const canDeleteRowInstant = (sectionIdx, rowIdx) => {
    const section = sectionData.rowSections[sectionIdx];
    if (!section || !section.rows[rowIdx]) return false;
    ...
    // All values are empty, can delete instantly
    return true;
  };
  ...
  const deleteColumn = (colIdx) => {
    ...
    // Make sure this is a valid column for deletion
  ...
  const handleKeyDown = (e, sectionIdx, rowIdx, colIdx, field) => {
    ...
    if (e.key === 'Delete' && e.shiftKey) {
      ...
      if (colIdx === 0) {
        // Delete entire row
        if (canDeleteRowInstant(sectionIdx, rowIdx)) {
          // If all values are empty, delete immediately
          deleteRow(sectionIdx, rowIdx);
        } else if (pendingDeleteRow === `${sectionIdx}-${rowIdx}`) {
          // If already pending, confirm deletion
          clearTimeout(pendingDeleteTimer.current);
          setPendingDeleteRow(null);
          deleteRow(sectionIdx, rowIdx);
        } else {
          // Otherwise mark for pending deletion
        ...
        if (canDeleteColumnInstant(colIdx)) {
          // If all values are empty, delete immediately
          deleteColumn(colIdx);
        } else if (pendingDeleteCol === `${colIdx}`) {
          // If already pending, confirm deletion
          clearTimeout(pendingDeleteColTimer.current);
    ...
    // Quickfill functionality removed for now
  }; 

  // Quickfill functionality has been removed for now and will be reintroduced later
                ...
                <div className="cell-row">
                  ...
                  <div className={`row-label ${row.name ? 'has-name' : ''} ${focusedNameField === `${sectionIdx}-${rowIdx}-0` ? 'name-input-focused' : ''}`}>
                    ...
                    <span
                      ...
                      onClick={() => {
                        // If row has only empty values, delete immediately
                        if (canDeleteRowInstant(sectionIdx, rowIdx)) {
                          deleteRow(sectionIdx, rowIdx);
                          return;
                        }
                        
                        // Otherwise use two-step deletion with highlighting
                            ...
                            <div className={`row-label ${cell.name ? 'has-name' : ''} ${focusedNameField === `${sectionIdx}-${rowIdx}-${colIdx}` ? 'name-input-focused' : ''}`}>
                              ...
                              {cellIdx === row.cells.length - 1 && (
                                <span
                                  ...
                                  onClick={() => {
                                    // If column has only empty values, delete immediately
                                    if (canDeleteColumnInstant(colIdx)) {
                                      deleteColumn(colIdx);
                                      return;
                                    }
